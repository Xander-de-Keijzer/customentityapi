 ...
 */
object Reflection {
    private val OBC_PREFIX = Bukkit.getServer()::class.java.getPackage().name
    private val VERSION = OBC_PREFIX.replace("org.bukkit.craftbukkit", "").replace(".", "")
    private val version = VERSION.split("_")[1].toInt()
        ...
        for (method in clazz.declaredMethods) {
            if ((methodName == null || method.name == methodName)
                ...
                return object : MethodInvoker {
                    override fun invoke(target: Any?, vararg arguments: Any): Any {
                        try {
